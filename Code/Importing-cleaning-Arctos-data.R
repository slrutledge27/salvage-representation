...
df7 <- df6 %>% mutate(coll_method = ifelse(coll_method_7 == "salvage","salvage", "active"))


#### split into "active" vs "salvage" datasets #######
df_salvage <- df7[which(df7$coll_method == "salvage"),]
df_active <- df7[which(df7$coll_method == "active"),]

write.csv(df_unknown, "./Arctos_unknown.csv")
write.csv(df_salvage, "./Arctos_SALVAGE.csv")
write.csv(df_active, "./Arctos_ACTIVE.csv")

#### pull out vectors of species to calculate overlap ####
unknown <- unique(df_unknown$scientific_name)
active <- unique(df_active$scientific_name)
salvage <- unique(df_salvage$scientific_name)


## split into salvage and active collecting datasets
